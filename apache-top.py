...
class ApacheStatusParser(HTMLParser):
    ...
    append = False # defineix si els seguents caracters s'han d'afegir o posar e                                                                                                                               n un altre camp
...
def print_screen(screen, url, show_scoreboard):
    ...
    while not end:
        try:
            ...
            # imprimim el header
            screen.addstr(0,0,data.performance_info_data[5].replace("Server upti                                                                                                                               me: ","Uptime:").replace(" days","d").replace(" day","d").replace(" hours","h").                                                                                                                               replace(" hour","h").replace(" minutes","m").replace(" minute","m").replace(" se                                                                                                                               conds","s").replace("second","s") + ", " + data.performance_info_data[3])
            screen.addstr(1,0,data.performance_info_data[7])
            screen.addstr(2,0,data.performance_info_data[8].replace("request","r                                                                                                                               eq").replace("second","sec") + ", Active/Idle: " + data.performance_info_data[9]                                                                                                                               .split()[0] + "/" + data.performance_info_data[9].split()[5])
            ...
            if show_scoreboard:
                ...
                screen.addstr(5,0,"  |  W Sending Reply, K Keepalive (read), D D                                                                                                                               NS Lookup             |")
                screen.addstr(6,0,"  |  C Closing connection, L Logging, G Grace                                                                                                                               fully finishing       |")
                screen.addstr(7,0,"  |  I Idle cleanup of worker, . Open slot wi                                                                                                                               th no current process |")
                screen.addstr(8,0,"  +------------------------------------------                                                                                                                               ----------------------+")
            ...
            if len(message) > 0:
                screen.addstr(y+5+num/width,0,message, curses.A_BOLD | curses.A_                                                                                                                               REVERSE)
                message = ""

            print_proceses(y+6+num/width,0,screen, data.proceses_data, columns=[                                                                                                                                1, 3, 5, 4, 11, 10, 12 ], sort=sort, reverse=reverse, width=width, show_only_ac                                                                                                                               tive=show_only_active )
            ...
            if c == "q":
                ...
                # ordenar per SS"
                sort = 5
                message = "Sort by Seconds since beginning of most recent reques                                                                                                                               t"
            elif c == "V":
                # ordenar per vhost
                sort = 11
...
def print_proceses(y,x,screen, proceses, columns, sort, reverse, width, show_onl                                                                                                                               y_active = True):
    ...
    if sort != None:
        for process in sorted(proceses, key=operator.itemgetter(sort), reverse=r                                                                                                                               everse):
            n += print_process(y+n,x,screen,process,columns,show_only_active,wid                                                                                                                               th)
    else:
